# Contributing to dsh-maestro-notifier

Thank you for contributing to **dsh-maestro-notifier** (`@ddtcorex/dsh-maestro-notifier`) — a pluggable notifier service for the DeepSeek Harness (Telegram first; registry open to further providers) packaged as a Cordis plugin.
...
- `fix/<topic>` — bug fixes
- `feat/<topic>` — new features / new providers
...
- **Scope:** optional, without the `dsh-maestro-` prefix — e.g. `feat(notifier):`, `fix(telegram):`, `docs(readme):`
...
pnpm test        # vitest run
pnpm build       # tsc — ensures lib/ is not stale, lib/index.js flat
...
This package is public (`"private": false` — requires explicit `false` on npm). Never set `"private": true` in `package.json`. Publishing uses `pnpm publish --access public`.
...
- Security vulnerabilities: use GitHub's private advisory reporting at `https://github.com/ddtcorex/dsh-maestro-notifier/security/advisories` — do not file a public issue.
